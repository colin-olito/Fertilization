...
generated quantities {
    ...
    real<lower=0> fit_rep;        // ...
 	...
 	for (i in 1:N) {
 		log_lik[i]   =  binomial_logit_lpmf(nS[i] | nT[i], X[i] * beta);  // log-likelihood for LOO calculations
 		y_rep[i]     =  binomial_rng(nT[i], inv_logit(X[i] * beta));      // Randomly generated data for posterior predictive checks
 		y_hat[i]     =  inv_logit(X[i]*beta);
